...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  start_adj = ((start + 9) // 10) * 10
  if start_adj <= stop:
    x = torch.arange(start=start_adj, end=stop + 1, step=10, dtype=torch.float64)
  else:
...
def reshape_practice(x):
  ...
  # Replace "pass" statement with your code
  y = x.view(2, 3, 4)
  y = y.transpose(0, 1).contiguous().view(3, 8)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
    B = x.shape[0]
    z = [torch.mm(x[i], y[i]) for i in range(B)]
    z = torch.stack(z)
  else:
    z = torch.bmm(x, y)
  #############################################################################
...
def normalize_columns(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  mu = x.sum(0) / x.shape[0]
  sigma = torch.sqrt(((x - mu) ** 2).sum(0) / (x.shape[0] - 1))
  y = (x - mu) / sigma
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
